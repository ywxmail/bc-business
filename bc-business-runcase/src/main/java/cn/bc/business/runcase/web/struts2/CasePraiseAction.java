...
public class CasePraiseAction extends FileEntityAction<Long, Case4Praise> {
	...
	protected PageOption buildFormPageOption(boolean editable) {
		return super.buildFormPageOption(editable).setWidth(825)
				.setMinWidth(250).setHeight(480).setMinHeight(200);
	}
	...
	protected void buildFormPageButtons(PageOption pageOption, boolean editable) {
		...
		if (editable && !readonly) {
			// 特殊处理结案按钮
			if (Case4Praise.STATUS_ACTIVE == getE().getStatus()
					&& !getE().isNew()) {
				ButtonOption buttonOption = new ButtonOption(
						getText("label.closefile"), null,
						"bc.casePraiseForm.closefile");
	...
	protected void initForm(boolean editable) throws Exception {
		...
		this.attachsUI = this.buildAttachsUI(this.getE().isNew(),
				this.isReadonly() || !editable, Case4Praise.ATTACH_TYPE, this
						.getE().getUid());
