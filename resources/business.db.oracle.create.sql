...
COMMENT ON COLUMN BS_CARMAN.CAR IS '车辆信息';
...
CREATE INDEX BSIDX_CARMAN_NAME ON BS_CARMAN (NAME);
...
COMMENT ON COLUMN BS_CONTRACT.MAIN IS '批号';
COMMENT ON COLUMN BS_CONTRACT.WORD_NO IS '文书号';
COMMENT ON COLUMN BS_CONTRACT.CODE IS '合同号';
COMMENT ON COLUMN BS_CONTRACT.TYPE_ IS '合同类型：如劳动合同、经济合同等';
COMMENT ON COLUMN BS_CONTRACT.OP_TYPE IS '操作类型';
...
COMMENT ON COLUMN BS_CONTRACT_LABOUR.INSURANCE_TYPE IS '社保险种(五金,工伤)';
...
COMMENT ON TABLE BS_CAR_DRIVER IS '人车关系';
COMMENT ON COLUMN BS_CAR_DRIVER.STATUS_ IS '状态：0-启用中,1-已禁用,2-已删除';
COMMENT ON COLUMN BS_CAR_DRIVER.DRIVER_ID IS '司机ID';
COMMENT ON COLUMN BS_CAR_DRIVER.OLD_CAR_ID IS '原车辆ID';
COMMENT ON COLUMN BS_CAR_DRIVER.OLD_MOTORCADE_ID IS '原车队ID';
COMMENT ON COLUMN BS_CAR_DRIVER.OLD_DRIVER_STATE IS '营运班次:如1-正班、2-副班、3-顶班';
COMMENT ON COLUMN BS_CAR_DRIVER.NEW_CAR_ID IS '新车辆ID';
COMMENT ON COLUMN BS_CAR_DRIVER.NEW_MOTORCADE_ID IS '新车队ID';
COMMENT ON COLUMN BS_CAR_DRIVER.NEW_DRIVER_STATE IS '营运班次:如1-正班、2-副班、3-顶班';
COMMENT ON COLUMN BS_CAR_DRIVER.MOVE_DATE IS '迁移日期';
COMMENT ON COLUMN BS_CAR_DRIVER.MOVE_TYPE IS '迁移类型';
COMMENT ON COLUMN BS_CAR_DRIVER.FROM_UNIT IS '原单位';
COMMENT ON COLUMN BS_CAR_DRIVER.TO_UNIT IS '现单位';
COMMENT ON COLUMN BS_CAR_DRIVER.HAND_PAPERS_DATE IS '交证日期';
COMMENT ON COLUMN BS_CAR_DRIVER.CANCEL_ID IS '注销单号';
...
ALTER TABLE BS_CAR_DRIVER ADD CONSTRAINT BSFK_CARDRIVER_MODIFIER FOREIGN KEY (MODIFIER_ID)
      REFERENCES BC_IDENTITY_ACTOR_HISTORY (ID);
ALTER TABLE BS_CAR_DRIVER ADD CONSTRAINT BSFK_CARDRIVER_OLD_CAR FOREIGN KEY (OLD_CAR_ID)
      REFERENCES BS_CAR (ID);
ALTER TABLE BS_CAR_DRIVER ADD CONSTRAINT BSFK_CARDRIVER_NEW_CAR FOREIGN KEY (NEW_CAR_ID)
      REFERENCES BS_CAR (ID);
ALTER TABLE BS_CAR_DRIVER ADD CONSTRAINT BSFK_CASEBASE_OLD_MOTORCADE FOREIGN KEY (OLD_MOTORCADE_ID)
      REFERENCES BS_MOTORCADE (ID);
ALTER TABLE BS_CAR_DRIVER ADD CONSTRAINT BSFK_CASEBASE_NEW_MOTORCADE FOREIGN KEY (NEW_MOTORCADE_ID)
      REFERENCES BS_MOTORCADE (ID);
ALTER TABLE BS_CAR_DRIVER ADD CONSTRAINT BSFK_CARDRIVER_CARMAN FOREIGN KEY (DRIVER_ID)
      REFERENCES BS_CARMAN (ID);
      CREATE INDEX BSIDX_CARDRIVER_CAR ON BS_CAR_DRIVER (NEW_CAR_ID);
...
CREATE TABLE BS_INDUSTRIAL_INJURY (
   ...
   CONSTRAINT BSPK_INDUSTRIAL_INJURY PRIMARY KEY (ID)
);
...
ALTER TABLE BS_INDUSTRIAL_INJURY ADD CONSTRAINT FK_BS_INDUS_REFERENCE_BS_CONTR FOREIGN KEY (CONTRACT_ID)
