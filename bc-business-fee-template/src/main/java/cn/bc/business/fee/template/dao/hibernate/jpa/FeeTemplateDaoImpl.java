...
public class FeeTemplateDaoImpl extends HibernateCrudJpaDao<FeeTemplate> implements
		FeeTemplateDao {
	...
	public List<Map<String, String>> getTemplate(){
	 ...
	 return	HibernateJpaNativeQuery.executeNativeSql(getJpaTemplate(), hql,null,new RowMapper<Map<String, String>>() {
	...
	public boolean isTemplateExistFee(Long id){
		Condition c = new AndCondition().add(new EqualsCondition("pid", id)).add(
				new EqualsCondition("status", BCConstants.STATUS_ENABLED));
		return this.createQuery().condition(c).count() > 0;
	}
	
	//检测费用是否属于模板
	public boolean isFeeBelong2Template(Long pid){
		Condition c = new AndCondition().add(new EqualsCondition("id", pid)).add(
				new EqualsCondition("status", BCConstants.STATUS_ENABLED));
		return this.createQuery().condition(c).count() > 0;
	}
	...
	public List<Map<String, String>> getFeeBelong2Template(Long pid){
		String hql="SELECT a.id,a.name,a.price,a.count_,a.pay_type,a.desc_";
		   ...
		   hql+=" and a.pid=?";
		 return	HibernateJpaNativeQuery.executeNativeSql(getJpaTemplate(), hql,new Object[]{pid}
